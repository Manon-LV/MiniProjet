...
def test_dqn(model_path, n_episodes=1000, size=10, n_obstacle=5):
    ...
    np.save('results/03/testqiteration_avg_rewards.npy', avg_rewards)
    np.save('results/03/testqiteration_success_rates.npy', np.array(success_rates))
    np.save('results/03/testqiteration_episode_rewards.npy', np.array(episode_rewards))
    np.save('results/03/testqiteration_steps_per_episode.npy', np.array(steps_per_episode))

if __name__ == '__main__':
    test_dqn('qiteration_model.pth', n_episodes=1000)